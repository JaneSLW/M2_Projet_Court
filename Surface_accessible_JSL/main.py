...
for atm_i in range(len(nv_atom_id)):
	...
	surf_per_atm.append(surface_points * (4 * np.pi * (vdw_rad[nv_atom_id[atm_i]] + H20_RADIUS)**2 ) / POINTS_SPHERE)	# accessible surface area per atm
	...
	if surface_points > 0:
		res_on_surface.append(residue_to_atm[atm_i])	# to only keep residues on surface

ASA = sum(ASA_per_atm)	# total ASA of protein

ASA_per_res = {}	# ASA per residue to calculate rASA 
for key, value in zip(res_on_surface, surf_per_atm):
...
for residue_i in range(len(residues)):
	if residue_i in list(ASA_per_res.keys()):
		rASA = rASA + ASA_per_res[residue_i] / max_asa[residues[residue_i]]	# rASA = ASA / max_asa_tot
