...
max_asa = {"ALA": 129, "ARG": 274, "ASN": 195, "ASP": 193, "CYS": 167,
		   ...
		   "SER": 155, "THR": 172, "TRP": 285, "TYR": 263, "VAL": 174}  # Maximum possible solvent accessible surface area in Angstrom2 ; Theorical Values. (Tien et al. 2013)
...
if args.name:
	...
	repository = Bio.PDB.PDBList(pdb=name_protein)
	repository.retrieve_pdb_file(name_protein, file_format='pdb', pdir='.')
...
for atm_i in range(len(nv_atom_id)):
	neighbour_atoms = np.where((distances_array[:][atm_i] < 10) & (distances_array[:][atm_i] > 0))[0]  # Row index of neighboor atoms.
	...
	for point in range(POINTS_SPHERE):
		...
		for neighbour_atom in neighbour_atoms:
			dist_point_center = np.sqrt((sphere_points_array[point][0] - atom_coords_array[neighbour_atom][0])**2 
				...
				+ (sphere_points_array[point][2] - atom_coords_array[neighbour_atom][2])**2)

			if(dist_point_center < (vdw_rad[nv_atom_id[neighbour_atom]] + H20_RADIUS)):	 # Distance threshold : neighboor atom's radius + H20(solvant) radius 
				free_point = 0
		...
		if(free_point > 0): 
			surface_points += 1	 # Number of free sphere's points (max = 92).
	tot_acc_area = tot_acc_area + (4 * np.pi * (vdw_rad[nv_atom_id[atm_i]] + H20_RADIUS)**2)  # Total area of solvated sphere (for accessibility percentage calculation).
	ASA_per_atm.append(surface_points * (4 * np.pi * (vdw_rad[nv_atom_id[atm_i]] + H20_RADIUS)**2 ) / POINTS_SPHERE)  # In Angstrom2.
	if surface_points > 0:
		res_on_surface.append(residue_to_atm[atm_i])  # To only keep residues on surface (and accelerate rASA calculation).


ASA = sum(ASA_per_atm)  # ASA of the protein.
...
for residue_i in range(len(residues)):
	if str(residue_i) in ASA_per_res.keys():
		if residues[residue_i] in max_asa.keys(): # To discard modified residues "HYP".
